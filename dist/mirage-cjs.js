...
var Pretender = _interopDefault(require('pretender'));
...
var invokeMap = _interopDefault(require('lodash.invokemap'));
...
class HasMany extends Association {
  get identifier() {
    if (typeof identifierCache$1[this.key] !== "string") {
      const identifier = `${camelize(this._container.inflector.singularize(this.key))}Ids`;
  ...
  getForeignKey() {
    // we reuse identifierCache because it's the same logic as get identifier
    if (typeof identifierCache$1[this.key] !== "string") {
      const foreignKey = `${this._container.inflector.singularize(camelize(this.key))}Ids`;
  ...
  addMethodsToModelClass(ModelClass, key) {
    ...
    */
    ...
    modelPrototype[`new${capitalize(camelize(this._container.inflector.singularize(association.key)))}`] = function (...args) {
      let modelName, attrs;
    ...
    */
    ...
    modelPrototype[`create${capitalize(camelize(this._container.inflector.singularize(association.key)))}`] = function (...args) {
      let modelName, attrs;
...
class Model {
  ...
  */
  ...
  _explicitInverseFor(association) {
    ...
    if (candidateInverse && candidate.opts.inverse !== association.key) {
      assert(false, `You specified an inverse of ${inverse} for ${association.key}, but it does not match ${candidate.modelName} ${candidate.key}'s inverse`);
    }
  ...
  */
  ...
  _checkForMultipleExplicitInverses(association) {
    ...
    assert(matchingExplicitInverses.length <= 1, `The ${this.modelName} model has defined multiple explicit inverse associations for the ${association.ownerModelName}.${association.key} association.`);
  }
  /**
  ...
  */
  ...
  _implicitInverseFor(association) {
    ...
    return values(associations).filter(candidate => candidate.modelName === modelName).reduce((inverse, candidate) => {
...
class Serializer {
  ...
  getHashForResource(resource, removeForeignKeys = false, didSerialize = {}, lookupSerializer = false) {
    ...
    if (this.embed) {
      ...
      let addToIncludes = uniqBy(compact(flatten(serializer.getKeysForIncluded().map(key => {
